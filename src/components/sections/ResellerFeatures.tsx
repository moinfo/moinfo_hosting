...
import { resellerFeatureSections, planNames } from "@/data/reseller-features";
...
function CellValue({ value }: { value: string }) {
  ...
  if (value.startsWith("✓ "))
    return (
      <span className={classes.checkWithNote}>
        <IconCheck size={18} className={classes.check} />
        <span className={classes.note}>{value.slice(2)}</span>
      </span>
    );
...
export function ResellerFeatures() {
  return (
    <section className={classes.section}>
      <Container size="xl">
        <Accordion variant="separated" radius="md" defaultValue="Reseller Plan Features">
          {resellerFeatureSections.map((section) => (
